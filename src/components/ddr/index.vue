<script>
import { getBoundingRect, getHandler, getPoints, getSize, handlerPointMap, heightMap, pointMap, pointMap2, rad2deg, tr2bl, widthMap } from './helper';
...
export default {
  ...
  methods: {
    ...
    allowTransform(rect) {
      ...
      return rect.left >= 0 && rect.right <= this._parentRect.width && rect.top >= 0 && rect.bottom <= this._parentRect.height;
    },
  ...
  render() {
    return (
      <div ref='wrapper' style={this.style} class={this.getClassNames()} onTouchstart={this.handleMouseDown} onMousedown={this.handleMouseDown}>
        {this.renderContent ? this.renderContent(this) : this.$slots.default}
        {this.resizable && (
          <div class='resize-handler-wrapper'>
            {this.resizeHandler.map((item) => {
              return <span data-resizetype={item} key={item} style={this.getNewHandler(item)} class={`resize-handler ${item}`} />;
            })}
          </div>
        )}
