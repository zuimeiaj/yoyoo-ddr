  ...
  methods: {
    getContains() {
      ...
      const crossComponents = controls.filter((item) => {
        let transform = getBoundingRect(item.transform, 1);
    ...
    handleSelectionEnd() {
      if (this.selectedComponents.length == 1) {
        this.application.handleSelect(this.application.controls.find((item) => item.id === this.selectedComponents[0].id));
        this.selectedComponents = [];
